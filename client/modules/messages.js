...
export class Messages {
    ...
    renderOwnMessage = (status, username, message, date, dateId) => {
        ...
        if (status === 'delivered') {
            const messageId = document.querySelector(`div[data-id='${dateId}'] .status`);
            setTimeout(() => {
                messageId.innerHTML = 'delivered';
            }, 200);
        } else {
            const ownMessage = `<div data-id="${dateId}"><span class=${className} <b>[${username} <small>${renderDate(date)}</small>]</b> ${message} </span><small class="status">sending ...</small></div>`;
